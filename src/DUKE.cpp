...
struct DUKECKD6 : BlueCKD6 {
	void onChange(const event::Change &e) override {
		...
		DUKE *module = dynamic_cast<DUKE*>(this->paramQuantity->module);
		if (parent && module) {
			if (this->paramQuantity->paramId == DUKE::ADONF_PARAM) {
				for (int i = 0; i < 4 ; i++) {
					parent->sliders[i]->paramQuantity->setValue(10);
					module->params[DUKE::SLIDER_PARAM + i].value = 10.0f;
				}
			} else if (this->paramQuantity->paramId == DUKE::NADA_PARAM) {
				for (int i = 0; i < 4 ; i++) {
					parent->sliders[i]->paramQuantity->setValue(0.0f);
...
DUKEWidget::DUKEWidget(DUKE *module) {
	setModule(module);
	...
	addParam(createParam<DUKECKD6>(Vec(portX0[0]-29.0f, 190.0f), module, DUKE::ADONF_PARAM));
	addParam(createParam<DUKECKD6>(Vec(portX0[0]-29.0f, 230.0f), module, DUKE::NADA_PARAM));
	...
	for (int i = 0; i < 4; i++) {
		...
		sliders[i] = createParam<BidooLongSlider>(Vec(portX0[i]+4.0f, 176.0f), module, DUKE::SLIDER_PARAM + i);
		addParam(sliders[i]);

...
//		
//		/*
//		//unsure what to replace with ouput is void, prehaps isConnected()
//		if (outputs[GATEBRIDGE_OUTPUT].value == 0.0f) lights[GATE_LIGHT].value -= 4 * lights[GATE_LIGHT].value * invLightLambda * invESR;
//		if (!isActive) lights[GATE_LIGHT + 2].setBrightness() -= 4 * lights[GATE_LIGHT + 2].value * invLightLambda * invESR;
...
//			cv->valueForDisplay = &module->params[MU::CV_PARAM].getValue();
//			cv->format = "%2.2f V";
//			cv->header = "CV";
//			addParam(cv);
//
...
//			bpmIn->valueForDisplay = &module->inputs[MU::BPM_INPUT].getVoltage();
//			bpmIn->format = "%2.2f V";
//			bpmIn->header = "BPM";
//			addInput(bpmIn);
...
//			bpmOut->valueForDisplay = &module->params[MU::BPM_PARAM].getValue() + params[MU::BPMFINE_PARAM].getValue() / 80.f;//Use bpm param + fine / 80
//			//bpmOut->valueForDisplay = &module->outputs[MU::BPM_OUTPUT].value;//Use bpm param + fine / 80
...
//			activateIn->valueForDisplay = &module->inputs[MU::ACTIVATE_INPUT].getVoltage();
...
//			activateOUT->valueForDisplay = &module->params[MU::BPM_PARAM].getValue() + params[MU::BPMFINE_PARAM].getValue() / 80.f;;//BPM + fine / 80
//			//activateOUT->valueForDisplay = 6.f/*&module->outputs[MU::BPM_OUTPUT].value*/;//BPM fine
...
//			activateAltOUT->valueForDisplay = 6.66f/*&module->outputs[MU::ALTEOSTEP_OUTPUT].value*/;//???
...
//			inhibitIn->valueForDisplay = &module->inputs[MU::INHIBIT_INPUT].getVoltage();
...
//			noteLengthIn->valueForDisplay = &module->inputs[MU::NOTELENGTH_INPUT].getVoltage();
...
//			offsetIn->valueForDisplay = &module->inputs[MU::OFFSET_INPUT].getVoltage();
...
//			cvIn->valueForDisplay = &module->inputs[MU::CV_INPUT].getVoltage();
//			cvIn->format = "%2.2f V";
//			cvIn->header = "CV mod";
//			addInput(cvIn);
//
...
//			numTrigsIn->valueForDisplay = &module->inputs[MU::NUMTRIGS_INPUT].getVoltage();
...
//			distTrigsIn->valueForDisplay = &module->inputs[MU::DISTTRIGS_INPUT].getVoltage();
...
//			gateBridgeIn->valueForDisplay = &module->inputs[MU::GATEBRIDGE_INPUT].getVoltage();
...
//			gateBridgeOut->valueForDisplay = 6.66f/*&module->outputs[MU::GATEBRIDGE_OUTPUT].value*/;//???
...
//			cvBridgeIn->valueForDisplay = &module->inputs[MU::CVBRIDGE_INPUT].getVoltage();
...
//			cvBridgeOut->valueForDisplay = 6.66f/*&module->outputs[MU::CVBRIDGE_OUTPUT].value*/;//???input.value

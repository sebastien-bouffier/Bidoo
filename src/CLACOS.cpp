...
//
// using namespace std;		//Used?? /**e.g. std::pow*/
//
// extern float sawTTable[2048];
// extern float triTTable[2048];
//
...
//
// 	dsp::Decimator<16, 16> mainDecimator;
// 	dsp::RCFilter sqrFilter;
// 	dsp::RCFilter mainFilter;
//
...
// 				const float pitchSlewTau = 100.0f;
...
// 				if (phaseDist < 0.5f)
// 					sinBuffer[i] = 1.0f - 16.0f * std::pow(phaseDist - 0.25f, 2.0f);
// 				else
// 					sinBuffer[i] = -1.0f + 16.0f * std::pow(phaseDist - 0.75f, 2.0f);
// 				sinBuffer[i] *= 1.08f;
// 			} else {
// 				sinBuffer[i] = std::sin(2.0f * M_PI * phaseDist);
// 			}
// 			if (analog) {
// 				triBuffer[i] = 1.25f * interpolateLinear(triTTable, phaseDist * 2047.f);
// 			} else {
// 				if (phaseDist < 0.25f)
...
// 			}
// 			if (analog) {
// 				sawBuffer[i] = 1.66f * interpolateLinear(sawTTable, phaseDist * 2047.f);
// 			} else {
// 				if (phaseDist < 0.5f)
...
// 			}
//
// 			waveFormIndex[index] = inputs[WAVEFORM_INPUT + index].isConnected() ?
// 				clamp((int)(rescale(inputs[WAVEFORM_INPUT + index].getVoltage(), 0.0f, 10.0f, 0.0f, 3.0f)), 0, 3) :
// 				clamp((int)(params[WAVEFORM_PARAM + index].getValue()), 0, 3);
//
...
// 					phaseDist = std::fmin(phaseDist + (deltaPhase / 16.0f) * phaseDistY[index] / phaseDistX[index], (index + 1)*0.25f);
// 				else
// 					phaseDist = std::fmin(phaseDist + (deltaPhase / 16.0f) * (1 - phaseDistY[index]) / (1 - phaseDistX[index]), (index + 1)*0.25f);
...
// 		///switch
// 		configParam<tpMode>(MODE_PARAM, 0.f, 1.f, 1.f, "Mode");
// 		configParam<tpSync>(SYNC_PARAM, 0.f, 1.f, 1.f, "Sync");
// 		///knob
// 		configParam(PITCH_PARAM, -54.f, 54.f, 0.f, "Frequency", " Hz", std::pow(2, 1 / 12.f), dsp::FREQ_C4);
...
// 		for (int i = 0; i < 4; i++) {
// 			configParam(WAVEFORM_PARAM + i, 0.f, 3.f, 0.f, "Wave Value");
// 		}
// 	}
//
...
// 		}
// 		setPitch(params[PITCH_PARAM].getValue(), pitchFine + pitchCv);
// 		syncEnabled = inputs[SYNC_INPUT].isConnected();
//
// 		process(args.sampleTime, inputs[SYNC_INPUT].getVoltage());
//
// 		// Set output
// 		outputs[MAIN_OUTPUT].setVoltage(5.0f * main());
// 	}//process
//
...
// 		for (int i = 0; i < 4; i++) {
// 			if ((!inputs[CLACOS::DIST_X_INPUT + i].isConnected()) && (!inputs[CLACOS::DIST_X_INPUT + i].isConnected())) {
// 				phaseDistX[i] = random::uniform();
// 				phaseDistY[i] = random::uniform();
// 			}
// 		}
// 	}
//
...
// 	CLACOSWidget(CLACOS *module) {
// 		setModule(module);
// 		setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/CLACOS.svg")));
//
...
// 					addInput(createInput<TinyPJ301MPort>(Vec(40.0f + 74.0f * (i % 2), 196.0f + 102.0f * std::round(i / 2)), module, CLACOS::DIST_X_INPUT + i));
// 					addInput(createInput<TinyPJ301MPort>(Vec(57.0f + 74.0f * (i % 2), 196.0f + 102.0f * std::round(i / 2)), module, CLACOS::DIST_Y_INPUT + i));
// 				}
// 			}
// 		}
//
// 		addParam(createParam<CKSS>(Vec(15.0f, 80.0f), module, CLACOS::MODE_PARAM));
// 		addParam(createParam<CKSS>(Vec(119.0f, 80.0f), module, CLACOS::SYNC_PARAM));
//
// 		addParam(createParam<BidooLargeBlueKnob>(Vec(57.0f, 45.0f), module, CLACOS::PITCH_PARAM));
// 		addParam(createParam<BidooBlueTrimpot>(Vec(114.0f, 45.0f), module, CLACOS::FINE_PARAM));
...
// };
//
// Model *modelCLACOS = createModel<CLACOS, CLACOSWidget>("clACos");
//
// float sawTTable[2048] = {
